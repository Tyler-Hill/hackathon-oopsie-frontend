...
def retrieve_tool_and_params_definition(conversation):
    ...
    interpretation = openai.ChatCompletion.create(
        ...
        prompt=(
            "You are an assistant that helps retrieve tool and parameter definitions from the conversation. "
        ...
        messages=conversation_formatted,
        temperature=0.5,
        max_tokens=100,
    )

    response = interpretation.choices[0].text.strip()
    return response
...
def pick_tool(conversation):
    interpretation = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a tool picker that identifies the appropriate action based on the user's conversation. "
            ...
            *conversation,
            {"role": "system", "content": "Here's the tool you've chosen:"},
        ],
        temperature=0.3,
        max_tokens=50,
    )
...
def extract_tool_parameters(tool_name, conversation):
    ...
    interpretation = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"You are an assistant that extracts relevant parameters from the conversation for the '{tool_name}' tool. "
            ...
            *conversation,
            {"role": "system", "content": "The final answer:"},
        ],
        temperature=0.3,
        max_tokens=100,
    )
